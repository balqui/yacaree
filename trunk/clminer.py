...
data in Dataset: scale, univ, nrtr, nrits, nrocc

ToDo:

Do I have a local Dataset or get one from the surrounding lattice?
...
class ClMiner:
    ...
    def computeclosures(self):
        ...
        if self.maxitemsupp < self.dataset.nrtr:
            ...
            yield(set2node(set(),self.dataset.nrtr))
            self.card += 1
...
## materialization moved to clattice

##    def materialize(self,suppthr=0):
##        self.closeds = [ e for e in takewhile(lambda c: c.supp >= suppthr, self.computecloseds()) ]

##    def materialize(self):
##        self.closeds = [ e for e in self.computecloseds() ]
